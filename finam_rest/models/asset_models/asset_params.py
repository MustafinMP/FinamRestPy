...
from finam_rest.models.money import MoneyModel
...
class AssetParams:
    ...
    shortable_status: ShortableStatus
    shortable_halted_days: int
    long_risk_rate: float
    long_collateral: MoneyModel
    short_risk_rate: float
    short_collateral: MoneyModel
    long_initial_margin: Optional[MoneyModel, None]
    short_initial_margin: Optional[MoneyModel, None]
    price_type: Optional[str | None]

    @classmethod
    def from_dict(cls, params_dict: dict) -> AssetParams:
        return AssetParams(
            ...
            long_collateral=MoneyModel.from_dict(params_dict['long_collateral']),
            short_risk_rate=float(params_dict['short_risk_rate']['value']),
            short_collateral=MoneyModel.from_dict(params_dict['short_collateral']),
            long_initial_margin=MoneyModel.from_dict(
                params_dict['long_initial_margin']) if 'long_initial_margin' in params_dict.keys() else None,
            short_initial_margin=MoneyModel.from_dict(
                params_dict['short_initial_margin']) if 'short_initial_margin' in params_dict.keys() else None,
            price_type=PriceType.from_str(params_dict['price_type']) if 'price_type' in params_dict else None
        )
