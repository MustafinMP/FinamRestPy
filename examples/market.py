...
bars = finam.market.get_bars(symbol, TimeFrame.TIME_FRAME_H1, now - TimeFrame.TIME_FRAME_H1.max_deep(), now)
for bar in bars:
    print(bar)

print('Get last quote example')
quote = finam.market.get_last_quote(symbol)
print(quote)

print('Get latest trades example')
trades = finam.market.get_latest_trades(symbol)
for trade in trades:
    print(trade)

print('Get order book example')
order_book = finam.market.get_order_book(symbol)
